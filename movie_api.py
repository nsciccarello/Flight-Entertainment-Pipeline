...
#Getting most popular movies at the moment
url = "https://imdb-api.com/en/API/MostPopularMovies/k_r76n2r9b"
response = requests.get(url)
print(response)
values=json.loads(response.text)
df=pd.DataFrame(values["items"])

#Adding more info to each popular movie
df2 = pd.DataFrame()
...
filename = f'movie_api_{current_date}.csv'
...
key = 'raw-movie-tv-show-data/raw-movie-data/' + filename
